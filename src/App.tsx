...
function App(): JSX.Element {
  ...
  const [shouldShowQuestions, setShouldShowQuestions] =
  ...
  useEffect(() => {
    ...
    if (useMock) {
      ...
      p = fetch("https://opentdb.com/api.php?amount=5").then((res) => {
    ...
    p.then((data) => {
      const formattedQuestions: FormattedQuestions[] = data.results.map(
        (question) => {
  ...
  //Derived values
  const score: number = questions.filter(
